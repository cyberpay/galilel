                       ...
                       int &nPaymentCount, int &nBlockStart, CBitcoinAddress &address, CAmount &nAmount)
{
    ...
    if (pindexPrev != NULL) nBlockMin = pindexPrev->nHeight - pindexPrev->nHeight % Params().Budget_SuperBlocks() + Params().Budget_SuperBlocks();

    nBlockStart = params[3].get_int();
    if (nBlockStart % Params().Budget_SuperBlocks() != 0) {
        int nNext = pindexPrev->nHeight - pindexPrev->nHeight % Params().Budget_SuperBlocks() + Params().Budget_SuperBlocks();
        throw std::runtime_error(strprintf("Invalid block start - must be a budget cycle block. Next valid block: %d", nNext));
    }

    int nBlockEnd = nBlockStart + (Params().Budget_SuperBlocks() * nPaymentCount); // End must be AFTER current cycle
...
UniValue preparebudget(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 6)
        throw std::runtime_error(
            "preparebudget \"proposal-name\" \"url\" payment-count block-start \"galilel-address\" monthy-payment\n"
            ...
            "5. \"galilel-address\":   (string, required) GALI address to send payments to\n"
            ...
            "\nExamples:\n" +
            HelpExampleCli("preparebudget", "\"test-proposal\" \"https://galilel.org/t/test-proposal\" 2 820800 \"UYB43zFEsQMmHEfZsHSZQ3ossbJ12f4xrp\" 500") +
            HelpExampleRpc("preparebudget", "\"test-proposal\" \"https://galilel.org/t/test-proposal\" 2 820800 \"UYB43zFEsQMmHEfZsHSZQ3ossbJ12f4xrp\" 500"));
    ...
    CWalletTx wtx;
    if (!pwalletMain->GetBudgetSystemCollateralTX(wtx, budgetProposalBroadcast.GetHash(), useIX)) { // 50 GALI collateral for proposal
...
UniValue submitbudget(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 7)
        throw std::runtime_error(
            "submitbudget \"proposal-name\" \"url\" payment-count block-start \"galilel-address\" monthly-payment \"fee-tx\"\n"
            ...
            "\nExamples:\n" +
            HelpExampleCli("submitbudget", "\"test-proposal\" \"https://galilel.org/t/test-proposal\" 2 820800 \"UYB43zFEsQMmHEfZsHSZQ3ossbJ12f4xrp\" 500") +
            HelpExampleRpc("submitbudget", "\"test-proposal\" \"https://galilel.org/t/test-proposal\" 2 820800 \"UYB43zFEsQMmHEfZsHSZQ3ossbJ12f4xrp\" 500"));
...
UniValue getbudgetprojection(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw std::runtime_error(
            ...
            "    \"PaymentAddress\": \"xxxx\",     (string) GALI address of payment\n"
