...
#if defined(NDEBUG)
#error "Galilel cannot be compiled without assertions."
#endif
...

    {
        ...
        if (tx.HasZerocoinSpendInputs()) {
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zGALI spend tx %s already in block %d",
            ...
            // Check that zGALI mints (if included) are not already known
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount, bool isZGALIStake)
{
    int64_t ret = 0;
...
void RecalculateZGALIMinted()
{
    ...
    while (true) {
        ...
        if (pindex->nHeight % 1000 == 0) {
            ...
            uiInterface.ShowProgress(_("Recalculating minted zGALI..."), percent);
        }
...
void RecalculateZGALISpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zGALI supply
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    //Track zGALI money supply in the block index
    if (!UpdateZGALISupply(block, pindex, fJustCheck))
        return state.DoS(100, error("%s: Failed to calculate new zGALI supply for block=%s height=%d", __func__,
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zGaliSpent: %s\n",
...
bool ContextualCheckZerocoinStake(int nHeight, CStakeInput* stake)
{
    if (nHeight < Params().Zerocoin_Block_V2_Start())
        return error("%s: zGALI stake block is less than allowed start height", __func__);

    if (CZGaliStake* zGALI = dynamic_cast<CZGaliStake*>(stake)) {
        CBlockIndex* pindexFrom = zGALI->GetIndexFrom();
        if (!pindexFrom)
            return error("%s: failed to get index associated with zGALI stake checksum", __func__);

        if (chainActive.Height() - pindexFrom->nHeight < Params().Zerocoin_RequiredStakeDepth())
            return error("%s: zGALI stake does not have required confirmation depth. Current height %d,  stakeInput height %d.", __func__, chainActive.Height(), pindexFrom->nHeight);
        ...
        if (nChecksum != zGALI->GetChecksum())
            return error("%s: accumulator checksum is different than the block %d blocks previous. stake=%d block240=%d", __func__, Params().Zerocoin_RequiredStakeDepth(), zGALI->GetChecksum(), nChecksum);
    } else {
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (block.IsProofOfStake()) {
        ...
        if (stake->IsZGALI() && !ContextualCheckZerocoinStake(pindexPrev->nHeight, stake.get()))
            return state.DoS(100, error("%s: staked zGALI fails context checks", __func__));
    ...
    if (isPoS) {
        ...
        // Inputs
        std::vector<CTxIn> galiInputs;
        std::vector<CTxIn> zGALIInputs;

        for (const CTxIn& stakeIn : stakeTxIn.vin) {
            if(stakeIn.IsZerocoinSpend()){
                zGALIInputs.push_back(stakeIn);
            }else{
                galiInputs.push_back(stakeIn);
            }
        }
        const bool hasGALIInputs = !galiInputs.empty();
        const bool hasZGALIInputs = !zGALIInputs.empty();
        ...
        for (const CTransaction& tx : block.vtx) {
            for (const CTxIn& in: tx.vin) {
                ...
                if(hasGALIInputs)
                    ...
                    for (const CTxIn& galiIn : galiInputs){
                        if(galiIn.prevout == in.prevout){
        ...
        if (isBlockFromFork) {
            ...
            while (!chainActive.Contains(prev)) {
                ...
                for (const CTransaction &t : bl.vtx) {
                    for (const CTxIn &in: t.vin) {
                        ...
                        for (const CTxIn &stakeIn : galiInputs) {
            ...
            // Now that this loop if completed. Check if we have zGALI inputs.
            if(hasZGALIInputs){
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zGALI mints and %d zGALI spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // GALI: We use certain sporks during IBD, so check to see if they are
        ...
        if(nLocalServices & NODE_BLOOM_LIGHT_ZC) {
            try {
                ...
                if (!GetAccumulatorValue(height, den, bnAccValue)) {
                    LogPrint("zgali", "peer misbehaving for request an invalid acc checkpoint \n", __func__);
                    Misbehaving(pfrom->GetId(), 50);
                } else {
            ...
            try {
                ...
                if (gen.isValid(chainActive.Height())) {
                    if (!lightWorker.addWitWork(gen)) {
                        LogPrint("zgali", "%s : add genwit request failed \n", __func__);
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
...
int ActiveProtocol()
{
    // SPORK_14 was used for 70722 (v3.3.0+).
    //if (IsSporkActive(SPORK_14_NEW_PROTOCOL_ENFORCEMENT))
    //        return MIN_PEER_PROTO_VERSION_AFTER_ENFORCEMENT;

    // SPORK_15 is used for 70723 (v3.4.0+).
