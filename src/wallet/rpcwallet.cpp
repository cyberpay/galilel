...
UniValue sendmany(const UniValue& params, bool fHelp)
{
    ...
    for (const std::string& name_ : keys) {
        ...
        if (!address.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid GALI address: ")+name_);
...
UniValue listmintedzerocoins(const UniValue& params, bool fHelp)
{
    ...
    CWalletDB walletdb(pwalletMain->strWalletFile);
    std::set<CMintMeta> setMints = pwalletMain->zgaliTracker->ListMints(true, fMatureOnly, true);
...
UniValue listzerocoinamounts(const UniValue& params, bool fHelp)
{
    ...
    CWalletDB walletdb(pwalletMain->strWalletFile);
    std::set<CMintMeta> setMints = pwalletMain->zgaliTracker->ListMints(true, true, true);
...
UniValue spendzerocoin(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 4 || params.size() < 3)
        throw std::runtime_error(
            ...
            "\nExamples\n" +
            HelpExampleCli("spendzerocoin", "5000 false true \"UjDM8Aw61soeAWRGQY9kcf1EDqyDjkpgSY\"") +
            HelpExampleRpc("spendzerocoin", "5000 false true \"UjDM8Aw61soeAWRGQY9kcf1EDqyDjkpgSY\""));
...
UniValue spendzerocoinmints(const UniValue& params, bool fHelp)
{
    ...
    return DoZgaliSpend(nAmount, false, true, vMintsSelected, address_str);
}


extern UniValue DoZgaliSpend(const CAmount nAmount, bool fMintChange, bool fMinimizeChange, std::vector<CZerocoinMint>& vMintsSelected, std::string address_str, bool ispublicspend)
{
...
UniValue resetmintzerocoin(const UniValue& params, bool fHelp)
{
    ...
    std::set<CMintMeta> setMints = zgaliTracker->ListMints(false, false, true);
...
UniValue resetspentzerocoin(const UniValue& params, bool fHelp)
{
    ...
    CWalletDB walletdb(pwalletMain->strWalletFile);
    CzGALITracker* zgaliTracker = pwalletMain->zgaliTracker.get();
    std::set<CMintMeta> setMints = zgaliTracker->ListMints(false, false, false);
...
UniValue exportzerocoins(const UniValue& params, bool fHelp)
{
    ...
    CzGALITracker* zgaliTracker = pwalletMain->zgaliTracker.get();
    std::set<CMintMeta> setMints = zgaliTracker->ListMints(!fIncludeSpent, false, false);
...
UniValue clearspendcache(const UniValue& params, bool fHelp)
{
    if(fHelp || params.size() != 0)
        throw std::runtime_error(
            "clearspendcache\n"
            "\nClear the pre-computed zGALI spend cache, and database.\n" +
            HelpRequiringPassphrase() + "\n"
...

    {
        int nTries = 0;
        while (nTries < 100) {
            TRY_LOCK(zgaliTracker->cs_spendcache, fLocked);
            if (fLocked) {
                if (zgaliTracker->ClearSpendCache()) {
