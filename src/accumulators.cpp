...
bool CalculateAccumulatorCheckpoint(int nHeight, uint256& nCheckpoint, AccumulatorMap& mapAccumulators)
{
    ...
    /* TODO: mbroemme@
     * This code is terrible buggy as it will not work with accumulator
     * checkpoints block height < 20 as InitializeAccumulators will return -1
     * and the worst is that it will crash the application rather than
     * throwing an error.
     */
...
bool ValidateAccumulatorCheckpoint(const CBlock& block, CBlockIndex* pindex, AccumulatorMap& mapAccumulators)
{
    ...
    //It is VERY IMPORTANT that when this is being run and height < v2_start, then zGALI need to be disabled at the same time!!
...
map<CoinDenomination, int> GetMintMaturityHeight()
{
    ...
    while (pindex && pindex->nHeight > Params().Zerocoin_Block_V2_Start()) {
