...
bool CheckProofOfStake(const CBlock block, uint256& hashProofOfStake, std::unique_ptr<CStakeInput>& stake, int nPreviousBlockHeight)
{
    ...
    if (txin.IsZerocoinSpend()) {
        ...
        stake = std::unique_ptr<CStakeInput>(new CZGaliStake(spend));

        if (!ContextualCheckZerocoinStake(nPreviousBlockHeight, stake.get()))
            return error("%s: staked zGALI fails context checks", __func__);
    } else {
        ...
        CGaliStake* galiInput = new CGaliStake();
        galiInput->SetInput(txPrev, txin.prevout.n);
        stake = std::unique_ptr<CStakeInput>(galiInput);
    }
    ...
    if (!txin.IsZerocoinSpend() && nPreviousBlockHeight >= Params().Zerocoin_Block_Public_Spend_Enabled() - 1) { //Equivalent for zGALI is checked above in ContextualCheckZerocoinStake()
